...
void main(void) {
    ...
    while(1){
        ...
        sensor1=conversion(0);
        temp=(sensor1*5.0)/1023.0;
        aux=temp;
        aux=-35.7*(aux*aux*aux)+440.7*aux*aux-1850*aux+2686;

        Lcd_Set_Cursor(1,1);
        Lcd_Write_String("NTC:");
        Lcd_flotante(aux);

        sensor2=conversion(1);
        temp=(sensor2*5.0)/1023;
        temp=(6*temp)+20;
        Lcd_Set_Cursor(1,10);
        Lcd_Write_String("LM35:");
        Lcd_Write_Int((int)temp);

        sensor3=conversion(2);
        temp=(sensor3*5.0)/1023;
        Lcd_Set_Cursor(2,1);
